...
var tunaWizard = {
    ...
    changeStep: function (currentStep, nextStep) {
        ...
        //Validations
        if (nextStep === 2) {
            if (typeof $("input[name='tn_gender']:checked").val() === "undefined") {
        ...
        if (nextStep === 3) {
            if (typeof $("input[name='tn_location']:checked").val() === "undefined") {
                self.setInputError($("input[name='tn_location']"));
                return;
            }
        }
        //Radio validation
        ...
        if (nextStep === self.stepCount) {
            ...
            stepConfirm.find("select[name='location[]']").selectpicker("val", $("input[name='tn_location']:checked").val());
            // stepConfirm.find("select[name='days[]']").selectpicker("val", $("input[name='tn_days']:checked").val());
            stepConfirm.find("input[name='zip']").val($("#tn_zip").val());

            var specialty = $("input[name='tn_specialty']:checked").map(function() {
                return this.value;
            }).get();
            stepConfirm.find("select[name='specialty[]']").selectpicker("val", specialty);

            var days = $("input[name='tn_days']:checked").map(function() {
                return this.value;
            }).get();
            stepConfirm.find("select[name='days[]']").selectpicker("val", days);

            var time = $("input[name='tn_time']:checked").map(function() {
                return this.value;
            }).get();
            stepConfirm.find("select[name='time[]']").selectpicker("val", time);
